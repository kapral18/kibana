...
import type { RemoteClusterAddTestBed } from './remote_clusters_add.helpers';
...
let testBed: RemoteClusterAddTestBed;
...
describe('Create Remote cluster', () => {
  ...
  describe('on component mount', () => {
    ...
    describe('Setup Trust', () => {
      ...
      test('next button should be disabled if not trust mode selected', () => {
    ...
    describe('Form step', () => {
      ...
      describe('on prem', () => {
        beforeEach(async () => {
          ...
          await actions.formStep.nameInput.setValue('remote_cluster_test');
        });
        ...
        test('server name has optional label', async () => {
          await actions.formStep.connectionModeSwitch.toggle();
        ...
        test('should display errors and disable the next button when clicking "next" without filling the form', async () => {
          await actions.formStep.nameInput.setValue('');
          ...
          await waitFor(() => {
            ...
            expect(actions.getErrorMessages()).toEqual(
              expect.arrayContaining([
                ...
                expect.stringContaining('At least one seed node is required'),
              ])
            );
        ...
        describe('seeds', () => {
          test('should only allow hostname, ipv4 and ipv6 format in the node "host" part', async () => {
            ...
            const expectInvalidChar = async (char: string) => {
              await actions.formStep.seedsInput.setValue(`192.16${char}:3000`);
              await waitFor(() => {
                expect(actions.getErrorMessages()).toEqual(
                  expect.arrayContaining([
            ...
            for (const char of [...NON_ALPHA_NUMERIC_CHARS, ...ACCENTED_CHARS].filter(
              notInArray(['-', '_', ':'])
            )) {
          ...
          test('should require a numeric "port" to be set', async () => {
            ...
            await actions.formStep.seedsInput.setValue('192.168.1.1');
            await waitFor(() => {
              expect(actions.getErrorMessages()).toEqual(
                expect.arrayContaining([expect.stringContaining('A port is required')])
              );
            });

            await actions.formStep.seedsInput.setValue('192.168.1.1:abc');
            await waitFor(() => {
              expect(actions.getErrorMessages()).toEqual(
                expect.arrayContaining([expect.stringContaining('A port is required')])
              );
        ...
        describe('node connections', () => {
          test('should require a valid number of node connections', async () => {
            ...
            await actions.formStep.nodeConnectionsInput.setValue(
              String(MAX_NODE_CONNECTIONS + 1)
            );
            await waitFor(() => {
              expect(actions.getErrorMessages()).toEqual(
                expect.arrayContaining([
        ...
        test('server name is optional (proxy connection)', async () => {
          ...
          await waitFor(() => {
            expect(actions.getErrorMessages()).toEqual(
              expect.arrayContaining([expect.stringContaining('A proxy address is required')])
            );
          });
        });
      });
      ...
      describe('on cloud', () => {
        ...
        test('TLS server name has optional label', async () => {
          await actions.formStep.cloudAdvancedOptionsSwitch.toggle();
          expect(actions.formStep.tlsServerNameInput.getLabel()).toBe(
        ...
        test('remote address is required', async () => {
          ...
          await waitFor(() => {
            expect(actions.getErrorMessages()).toEqual(
              expect.arrayContaining([expect.stringContaining('A remote address is required')])
            );
          });
        });
        ...
        test('should only allow alpha-numeric characters and "-" (dash) in the remote address "host" part', async () => {
          ...
          const expectInvalidChar = async (char: string) => {
            await actions.formStep.cloudRemoteAddressInput.setValue(`192.16${char}:3000`);
            await waitFor(() => {
              expect(actions.getErrorMessages()).toEqual(
                expect.arrayContaining([expect.stringContaining('Remote address is invalid')])
              );
            });
          };
      ...
      describe('form validation', () => {
        describe('remote cluster name', () => {
          test('should not allow spaces', async () => {
            await actions.formStep.nameInput.setValue('with space');
            ...
            await waitFor(() => {
              expect(actions.getErrorMessages()).toEqual(
                expect.arrayContaining([
                  expect.stringContaining('Spaces are not allowed in the name'),
                ])
              );
            });
          });
          ...
          test('should only allow alpha-numeric characters, "-" (dash) and "_" (underscore)', async () => {
            const expectInvalidChar = async (char: string) => {
              ...
              await actions.formStep.nameInput.setValue(`with${char}`);
              await waitFor(() => {
                expect(actions.getErrorMessages()).toEqual(
                  expect.arrayContaining([
                    expect.stringContaining(`Remove the character ${char} from the name`),
                  ])
                );
              });
            };
            ...
            for (const char of [...NON_ALPHA_NUMERIC_CHARS, ...ACCENTED_CHARS]) {
              await expectInvalidChar(char);
            }
          });
        });
        ...
        describe('proxy address', () => {
          ...
          test('should only allow alpha-numeric characters and "-" (dash) in the proxy address "host" part', async () => {
            ...
            const expectInvalidChar = async (char: string) => {
              await actions.formStep.proxyAddressInput.setValue(`192.16${char}:3000`);
              await waitFor(() => {
                expect(actions.getErrorMessages()).toEqual(
                  expect.arrayContaining([
                    expect.stringContaining(
          ...
          test('should require a numeric "port" to be set', async () => {
            ...
            await actions.formStep.proxyAddressInput.setValue('192.168.1.1');
            await waitFor(() => {
              expect(actions.getErrorMessages()).toEqual(
                expect.arrayContaining([expect.stringContaining('A port is required')])
              );
            ...
            await actions.formStep.proxyAddressInput.setValue('192.168.1.1:abc');
            await waitFor(() => {
              expect(actions.getErrorMessages()).toEqual(
                expect.arrayContaining([expect.stringContaining('A port is required')])
              );
